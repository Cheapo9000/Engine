...
#if PLATFORM_IOS || PLATFORM_MAC || (PLATFORM_ANDROID && USE_ANDROID_JNI)
...
bool FWebBrowserSingleton::Tick(float DeltaTime)
{
	...
	PreviousTickTimeSeconds = FPlatformTime::Seconds();
	...
	if (CEFBrowserApp != nullptr)
	{
		...
		// Set the external tick-call wait time to be in the acceptable range, but leaning heavily toward the longer time. That way, the ticker in
		// this class is likely to tick itself first, whenever possible. If the ticker is suspended (see where this is used outside of this class),
		// then this should be an acceptable wait time.
		static constexpr float ExternalTickCallWaitSecondsScale = 0.95f; 
		ExternalTickCallWaitSeconds = ExternalTickCallWaitSecondsScale * MaxForcedMessageLoopSeconds;
...
#if WITH_CEF3
double FWebBrowserSingleton::GetPreviousTickTimeSeconds() const
{
	return PreviousTickTimeSeconds;
}
#endif

#if WITH_CEF3
float FWebBrowserSingleton::GetExternalTickCallWaitSeconds() const
{
	static constexpr float DefaultExternalTickCallWaitSeconds = 0.064f;
	return (ExternalTickCallWaitSeconds <= 0.0f ? DefaultExternalTickCallWaitSeconds : ExternalTickCallWaitSeconds);
}
#endif
...
FString FWebBrowserSingleton::GenerateWebCacheFolderName(const FString& InputPath)
{
...
#if CEF_VERSION_MAJOR >= 128
	// CEF v128 prevents concurrent browser cache access between multiple processes.
	// It locks the cache folder by creating a "lockfile" file in it during initialization,
	// so that other CEF initializations will fail as long as the cache folder is locked.
	// The lockfile is then deleted when the locking CEF process ends (even in case of a crash)
	for (uint32 CacheNumber = 0; CacheNumber < 32; ++CacheNumber) {
		...
		if (!FPaths::FileExists(FPaths::Combine(ConcurrentCachePath, TEXT("lockfile"))))
		{
