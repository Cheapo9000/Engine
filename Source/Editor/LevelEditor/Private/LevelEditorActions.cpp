	...
	if (!JsonFile.IsEmpty())
	{
		...
		if (PlatformEngineIni && PlatformEngineIni->GetString(TEXT("DeviceProfileManager"), TEXT("PreviewDeviceProfileSelectionModule"), DeviceProfileSelectionModule, GEngineIni))
		{
			if (IDeviceProfileSelectorModule* DPSelectorModule = FModuleManager::LoadModulePtr<IDeviceProfileSelectorModule>(*DeviceProfileSelectionModule))
			{
				...
				{
					UE_LOG(LevelEditorActions, Warning, TEXT("JsonFile %s did not match any MatchProfile"), *JsonFile);
				}
			}
		}
	}
...
void FLevelEditorActionCallbacks::SetMaterialQualityLevel( EMaterialQualityLevel::Type NewQualityLevel )
{
	...
	EConsoleVariableFlags MaxPriority = GEditor && GEditor->IsFeatureLevelPreviewActive() ? ECVF_SetByPreview: ECVF_SetByScalability;
	MaterialQualityLevelVar->ReplaceCurrentPriorityAndTag(NewQualityLevel, MaxPriority);
...
bool FLevelEditorActionCallbacks::CanHideSelectedActors()
{
	for ( FSelectionIterator It( GEditor->GetSelectedActorIterator() ) ; It ; ++It )
	{
		AActor* Actor = static_cast<AActor*>( *It );
...
bool FLevelEditorActionCallbacks::CanShowSelectedActors()
{
	for ( FSelectionIterator It( GEditor->GetSelectedActorIterator() ) ; It ; ++It )
	{
		AActor* Actor = static_cast<AActor*>( *It );
...
bool FLevelEditorActionCallbacks::CanToggleSelectedActorsVisibility()
{
	return GEditor->GetSelectedActorCount() > 0;
}
...
{
	UI_COMMAND( BrowseDocumentation, "Level Editor Documentation", "Details on how to use the Level Editor", EUserInterfaceActionType::Button, FInputChord( EModifierKey::Shift, EKeys::F1 ) );
	...
	UI_COMMAND( BuildVirtualTextureOnly, "Build Streaming Runtime Virtual Textures", "Build runtime virtual texture low mips streaming data", EUserInterfaceActionType::Button, FInputChord());
	...
	UI_COMMAND( ShowSelected, "Show Selected", "Shows the selected actors", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( HideSelected, "Hide Selected", "Hides the selected actors", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( ToggleSelectedVisibility, "Toggle Selected Visibility", "Hides or shows the selected actors", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( ShowSelectedHierarchy, "Show Selected Hierarchy", "Shows the selected actors and their children", EUserInterfaceActionType::Button, FInputChord( EModifierKey::Shift, EKeys::H ) );
	UI_COMMAND( HideSelectedHierarchy, "Hide Selected Hierarchy", "Hides the selected actors and their children", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( ToggleSelectedHierarchyVisibility, "Toggle Selected Hierarchy Visibility", "Hides or shows the selected actors and their children", EUserInterfaceActionType::Button, FInputChord( EKeys::H ) );
	...
	UI_COMMAND( ShowSelectedHierarchyStartup, "Show Selected Hierarchy At Startup", "Shows selected actors and their current children at startup", EUserInterfaceActionType::Button, FInputChord() );
	UI_COMMAND( HideSelectedHierarchyStartup, "Hide Selected Hierarchy At Startup", "Hide selected actors and their current children at startup", EUserInterfaceActionType::Button, FInputChord() );
	...
	UI_COMMAND( ShowTransformWidget, "Show Transform Gizmo", "Toggles the visibility of the transform widgets", EUserInterfaceActionType::ToggleButton, FInputChord() );
	...
	for (const FPreviewPlatformMenuItem& Item : FDataDrivenPlatformInfoRegistry::GetAllPreviewPlatformMenuItems())
	{
		...
		if (!IsRunningCommandlet() && !GUsingNullRHI)
		{
			...
			else
			{
				...
				{
					FriendlyNameBuilder.AppendLine(FDataDrivenShaderPlatformInfo::GetFriendlyName(ShaderPlatform, Item.DeviceProfileName));
				}
			}
		}
