		...
		FRDGBufferSRVRef& OutTangent)
	{
		...
		if (!HashTable.IsValid(Index))
		{
			...
			if (const FRHIShaderResourceView* TangentSRV = InLODData->StaticVertexBuffers.StaticMeshVertexBuffer.GetTangentsSRV())
			{
				...
				Data.TangentBuffer = GraphBuilder.CreateBuffer(FRDGBufferDesc::CreateBufferDesc(TangentStride, TangentElementCount), TEXT("Hair.SkinnedDeformedTangents"));
	...
	FGlobalShaderMap* ShaderMap)
{
	...
	#if RHI_RAYTRACING
	if (IsHairRayTracingEnabled() && IsRayTracingEnabled() && !bIsReflectionCapture)
	{
		for (uint32 InstanceIndex : CardInstances)
		{
			FInstanceData& InstanceData = InstanceDatas[InstanceIndex];
