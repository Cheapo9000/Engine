...
#define FID_Engine_Plugins_Animation_ControlRig_Source_ControlRig_Public_Units_Control_RigUnit_Control_h_16_GENERATED_BODY \
	...
	{ \
		FEulerTransform& Transform = *(FEulerTransform*)RigVMMemoryHandles[0].GetPrivateData(RigVMExecuteContext.GetSlice().GetIndex()); \
		const FTransform& Base = *(FTransform*)RigVMMemoryHandles[1].GetInputData(); \
		const FTransform& InitTransform = *(FTransform*)RigVMMemoryHandles[2].GetInputData(); \
		FTransform& Result = *(FTransform*)RigVMMemoryHandles[3].GetOutputData(); \
		const FTransformFilter& Filter = *(FTransformFilter*)RigVMMemoryHandles[4].GetInputData(); \
