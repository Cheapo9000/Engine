...
#define FID_Engine_Plugins_Experimental_LiveLinkControlRig_Source_LiveLinkControlRig_Public_LiveLinkRigUnits_h_129_GENERATED_BODY \
	...
	{ \
		const FName& SubjectName = *(FName*)RigVMMemoryHandles[0].GetData(false); \
		const FName& PropertyName = *(FName*)RigVMMemoryHandles[1].GetData(false); \
		float& Value = *(float*)RigVMMemoryHandles[2].GetData(false); \
		 \
		StaticExecute( \
			RigVMExecuteContext.GetPublicData<FControlRigExecuteContext>(), \
...
template<> LIVELINKCONTROLRIG_API UScriptStruct* StaticStruct<struct FRigUnit_LiveLinkEvaluateBasicValue>();
