...
from learning_agents.train_common import create_task_dir, get_experiment_trackers
...
if __name__ == '__main__':
    ...
    parser.add_argument('--run-name', help='Optional run name/id under the training task.')
    parser.add_argument('-p', '--trainer-path', help='Optional path where custom trainer files can be found')
    parser.add_argument('-m', '--trainer-module', help='Optional module where custom trainer can be found', default='learning_agents.train_ppo')
    ...
    parser_socket.add_argument('--batch-size', type=int, default=1, help='Batch size to fill before a training step')

    # File Parser
    parser_socket = subparsers.add_parser('File', help='Train using file system')
    parser_socket.add_argument('data_config_path', help='Path of data config json that describes network schemas. (The full s3 path if using s3)')
    parser_socket.add_argument('trainer_config_path', help='Path of trainer config json that describes training details. (The full s3 path if using s3)')
    parser_socket.add_argument('network_dir', help='Directory of network binaries. (The full s3 path to the directory if using s3)')
    parser_socket.add_argument('replay_dir', help='Directory of replay binaries. (The full s3 path to the directory if using s3)')
    parser_socket.add_argument('temp_directory', help='Temporary directory where this process can create folders and save files')
    parser_socket.add_argument('--storage-service', type=str, default='os', help='File storage service. \'os\' to use files locally or \'s3\' to read from AWS Simple Storage Service.')
    ...
    if trainer_module == "train_ppo":
        trainer_module = "learning_agents.train_ppo"
        logger.warning("Using 'train_ppo' is deprecated, please use 'learning_agents.train_ppo'")
    
    # Import the trainer module and find the train function
    module = import_module(trainer_module)
    ...
    if args.communication_type == 'SharedMemory':
        ...
        from learning_agents.communicators.shared_memory_communicator import SharedMemoryCommunicator
        ...
        from learning_agents.communicators.socket_communicator import SocketCommunicator
        ...
        with SocketCommunicator(host, port, config['BatchSize']) as communicator:
            logger.info(f'Socket Communicator| Batch Size: {args.batch_size}')
    ...
    elif args.communication_type == 'File':
        from learning_agents.communicators.file_communicator import FileCommunicator
        logger.info('Starting File Communicator...')
        
        if args.storage_service == 's3':
            from learning_agents.communicators.file_communicator import FileManagerS3
            file_manager = FileManagerS3(args.data_config_path, args.trainer_config_path, args.network_dir, args.replay_dir)
        elif args.storage_service == 'os':
            from learning_agents.communicators.file_communicator import FileManagerOS
            file_manager = FileManagerOS(args.data_config_path, args.trainer_config_path, args.network_dir, args.replay_dir)
        else:
            raise Exception('Unknown File Manager Type introduced to File Communicator %s' % args.storage_service)

        communicator = FileCommunicator(file_manager)
        ...
        train(communicator.config, communicator, get_experiment_trackers(config))
