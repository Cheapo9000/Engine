...
from learning_agents.train_common import Profile, AbstractExperimentTracker, AbstractCommunicator
from learning_agents.train_common import save_snapshot_to_file, schema_noise_mask_observation
from learning_agents.train_common import UE_RESPONSE_SUCCESS
...
def train(config, communicator:AbstractCommunicator, trackers: list[AbstractExperimentTracker] = []):
    ...
    from learning_agents.train_common import schema_act_num, schema_entropy, schema_log_prob, schema_regularization
    ...
    lr_step_size = bc_config['LearningRateDecayStepSize']
    ...
    observation_noise_scale = bc_config['ObservationNoiseScale']
    ...
    iterations_per_snapshot = bc_config['IterationsPerSnapshot']
    ...
    while ti <= niterations:
        ...
        for _ in range(iterations_per_sync):
            ...
            with Profile('Training'):
                ...
                batch_obs = observations[window_indices[batch]] # [batch_size, window, obs dim]
                batch_act = actions[window_indices[batch]] # [batch_size, window, action dim]
                
                batch_obs_noise_mask = schema_noise_mask_observation(observation_schema, batch_obs.reshape(-1, batch_obs.shape[-1])).view_as(batch_obs)
                batch_obs_noise = batch_obs_noise_mask * torch.randn_like(batch_obs) * torch.rand(batch_size, device=device)[:,None,None] * observation_noise_scale
                ...
                for i in range(window):
                    ...
                    act_enc_policy_mem = policy_network(torch.cat([encoder_network(batch_obs[:,i] + batch_obs_noise[:,i]), policy_mem], dim=-1))
            ...
            if save_snapshots and ti % iterations_per_snapshot == 0:
                ...
                for tracker in trackers:
                    ...
                    tracker.track_snapshots(snapshots, str(ti))
            
            ti += 1

            # Update lr schedulers

            if ti % lr_step_size == 0:
