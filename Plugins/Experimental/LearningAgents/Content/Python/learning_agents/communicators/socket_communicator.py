...
from learning_agents.train_common import save_snapshot, load_snapshot, build_network, get_alive_threads, get_merged_buffer, get_merged_stats
...
class SocketCommunicator(AbstractCommunicator):
    ...
    def __init__(self, host, port, batch_size=1, connection_waiting_timeout=None, discard_stale_experiences=True):        
        self.name = 'socket'
        ...
        self.min_batch_size = batch_size if batch_size else 1
        self.max_batch_size = batch_size if batch_size else 1
                    ...
                    with self.lock:
                        ...
                        if self.discard_stale_experiences:
                            if networks_version < self.networks_version: 
                                logger.debug(f"Discarding stale buffer with networks version [{networks_version}] from connection {socket_name}; Latest networks version is [{self.networks_version}]")
                                continue
        ...
        while self.listener_active:
            try:
                ...
                with self.lock:
                    ...
                    logger.info(f"Added {len(self.client_sockets)}-th client socket!")

                    # Do not start the worker thread if BehaviorCloning because it will consume the experiences before the training thread can get them
                    if 'BehaviorCloningSettings' not in self.config:
                ...
                for client_socket in self.client_sockets:
                    try:
                        if client_socket.ready_to_receive_networks and client_socket.networks_version < self.networks_version:
                            logger.debug(f"Sending networks to connection {client_socket.name}... Network IDs: {list(self.networks_by_id.keys())} | Version: {self.networks_version}")
                            ...
                            socket_send_ping(client_socket()) # heartbeat
                    except ConnectionError as e:
                        self._handle_connection_error(client_socket)
