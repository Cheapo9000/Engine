...
def load_snapshot_from_file(network, filename):
    with open(filename, 'rb') as f:
        data = np.frombuffer(f.read(), dtype=np.uint8)
        load_snapshot(data, network)
...
def independent_normal_log_prob(mean, log_std, value, masked):
    ...
    return (1.0 - masked) * (-((value - mean) ** 2) / (2 * (std ** 2)) - log_std_clipped - log_sqrt_2pi)
...
class MLFlowTracker(AbstractExperimentTracker):
    ...
    def initialize_tracker(self):
        try:
            import mlflow
            tracking_uri = self.config.get('MLflowTrackingUri')
            # Set the tracking server URI
            logger.info(f'Logging to MLflow Uri {tracking_uri}...')
            self.tracker = mlflow
            mlflow.set_tracking_uri(tracking_uri)
        except ImportError as e:
            logger.warning('Failed to Load MLflow : %s. Please add manually to site-packages.' % str(e))
            self.config['UseMLflow'] = False
            self.tracking = False
    
    
        if self.tracking:
            self.tracker.set_experiment(self.config["TaskName"])
            self.tracker.start_run(run_name=self.config.get("RunName"))
    ...
    def track_snapshots(self, data: list, dir: str):
        if self.tracking:
            for d in data:
                self.tracker.log_artifact(d, dir)
