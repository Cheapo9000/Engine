...
class GRUCell(nn.Module):
    ...
    def __init__(self, 
        ...
        self.output_size = output_size
        self.Wr = remember_layer
        self.Wz = update_layer
        self.Wh = activation_layer
    ...
    def forward(self, x):
        ...
        # Input Mask
        x_mask = torch.cat([torch.ones([len(x), self.input_size], device=x.device), r], dim=-1)
        
        # Output
        y = (1 - z) * x[:,self.input_size:] + z * torch.tanh(self.Wh(x_mask * x))
...
class Conv1d(nn.Module):
    ...
    def __init__(self, input_length, in_channels, out_channels, kernel_size, padding, padding_mode, sublayer, device):
        super(Conv1d, self).__init__()
        self.weight = nn.Parameter(torch.zeros([out_channels, in_channels, kernel_size], device=device))
    ...
    def forward(self, x):
        # x dimensions should be (batch_size, in_channels * input_length)
        batch_size = x.shape[0]

        sublayer_out = self.sublayer(x)
        x = sublayer_out.reshape(batch_size, self.in_channels, self.input_length)
        padded_in = F.pad(x, (self.padding, self.padding), mode=padding_modes[self.padding_mode])

        return F.conv1d(padded_in, self.weight, self.bias).reshape(batch_size, -1)

class Conv2d(nn.Module):

    def __init__(self, in_height, in_width, in_channels, out_channels, kernel_size, stride, padding, padding_mode, sublayer, device):
        super(Conv2d, self).__init__()
        self.weight = nn.Parameter(torch.zeros([out_channels, in_channels, kernel_size, kernel_size], device=device))
        self.bias = nn.Parameter(torch.zeros([out_channels], device=device))
    ...
    def forward(self, x):
        # x dimensions should be (batch_size, in_channels * in_height * in_width)
        batch_size = x.shape[0]

        sublayer_out = self.sublayer(x)
        x = sublayer_out.reshape(batch_size, self.in_channels, self.in_height, self.in_width)
        padded_in = F.pad(x, (self.padding, self.padding, self.padding, self.padding), mode=padding_modes[self.padding_mode])

        return F.conv2d(padded_in, self.weight, self.bias, self.stride).reshape(batch_size, -1)
...
def update_pytorch_module_from_nne_data(module, nne_data):
    ...
    with torch.no_grad():
        ...
        elif nne_data['Type'] == 'GRUCell':
            assert isinstance(module, GRUCell)
            update_pytorch_module_from_nne_data(module.Wr, nne_data['RememberLayer'])
            update_pytorch_module_from_nne_data(module.Wz, nne_data['UpdateLayer'])
            update_pytorch_module_from_nne_data(module.Wh, nne_data['ActivationLayer'])

        elif nne_data['Type'] == 'Conv1d':
            assert isinstance(module, Conv1d)
            module.weight.data[:] = torch.as_tensor(nne_data['Weights'])
            module.bias.data[:] = torch.as_tensor(nne_data['Biases'])
            update_pytorch_module_from_nne_data(module.sublayer, nne_data['SubLayer'])

        elif nne_data['Type'] == 'Conv2d':
...
def create_pytorch_module_from_nne_data(nne_data, device):
    ...
    if nne_data['Type'] == 'Linear':
        ...
        module = GRUCell(
            ...
            output_size=nne_data['OutputSize'],
            remember_layer=create_pytorch_module_from_nne_data(nne_data['RememberLayer'], device=device), 
            update_layer=create_pytorch_module_from_nne_data(nne_data['UpdateLayer'], device=device), 
            activation_layer=create_pytorch_module_from_nne_data(nne_data['ActivationLayer'], device=device))

    elif nne_data['Type'] == 'Conv1d':
        module = Conv1d(
            input_length=nne_data['InputLength'], 
            in_channels=nne_data['InChannels'],
            out_channels=nne_data['OutChannels'],
            kernel_size=nne_data['KernelSize'],
            padding=nne_data['Padding'],
            padding_mode=nne_data['PaddingMode'],
            sublayer=create_pytorch_module_from_nne_data(nne_data['SubLayer'], device=device),
            device=device)
    
    elif nne_data['Type'] == 'Conv2d':
        module = Conv2d(
            in_height=nne_data['InputHeight'], 
            in_width=nne_data['InputWidth'], 
            in_channels=nne_data['InChannels'],
            out_channels=nne_data['OutChannels'],
            kernel_size=nne_data['KernelSize'],
            stride=nne_data['Stride'],
            padding=nne_data['Padding'],
            padding_mode=nne_data['PaddingMode'],
            sublayer=create_pytorch_module_from_nne_data(nne_data['SubLayer'], device=device),
            device=device)
    ...
    with torch.no_grad():
        ...
        if isinstance(module, ReLU): return { 'Type': 'ReLU', 'Size': module.size }
            ...
            return {
                ...
                'OutputSize': module.output_size,
                'RememberLayer': get_nne_data_from_pytorch_module(module.Wr),
                'UpdateLayer': get_nne_data_from_pytorch_module(module.Wz),
                'ActivationLayer': get_nne_data_from_pytorch_module(module.Wh),
            }
            ...
            return {
                ...
                'Weights': module.weight.cpu().detach().numpy().astype(np.float32),
                'Biases': module.bias.cpu().detach().numpy().astype(np.float32),
                "InputHeight": module.in_height,
                "InputWidth": module.in_width,
                "InChannels": module.in_channels,
                "OutChannels": module.out_channels,
                "KernelSize": module.kernel_size,
