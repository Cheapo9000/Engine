...
def compress_weights(weights, eps=1e-8):
    ...
    if weights.shape[-1] > 0:
        ...
        compressed = np.zeros_like(weights, dtype=np.uint16)
        offsets = np.zeros([weights.shape[0]], dtype=np.float32)
        scales = np.ones([weights.shape[0]], dtype=np.float32)
        return compressed, offsets, scales
...
def serialization_size_layer(offset, layer):
    ...
    if layer_type == "Invalid":
        ...
        offset = serialization_size_uint32(offset)
        offset = serialization_size_layer(offset, layer["RememberLayer"])
        offset = serialization_size_layer(offset, layer["UpdateLayer"])
        offset = serialization_size_layer(offset, layer["ActivationLayer"])
...
def serialization_save_layer(offset, layer, buffer):
    ...
    if layer_type == "Invalid":
        ...
        offset = serialization_save_uint32(offset, layer["OutputSize"], buffer)
        offset = serialization_save_layer(offset, layer["RememberLayer"], buffer)
        offset = serialization_save_layer(offset, layer["UpdateLayer"], buffer)
        offset = serialization_save_layer(offset, layer["ActivationLayer"], buffer)
    
    elif layer_type == "Conv1d":
        offset = serialization_save_uint32(offset, layer["InputLength"], buffer)
        offset = serialization_save_uint32(offset, layer["InChannels"], buffer)
        offset = serialization_save_uint32(offset, layer["OutChannels"], buffer)
        offset = serialization_save_uint32(offset, layer["KernelSize"], buffer)
        offset = serialization_save_uint32(offset, layer["Padding"], buffer)
        offset = serialization_save_uint32(offset, layer["PaddingMode"], buffer)
        offset = serialization_save_float_array(offset, layer["Weights"], buffer)
        offset = serialization_save_float_array(offset, layer["Biases"], buffer)
        offset = serialization_save_layer(offset, layer["SubLayer"], buffer)

    elif layer_type == "Conv2d":
        offset = serialization_save_uint32(offset, layer["InputHeight"], buffer)
        offset = serialization_save_uint32(offset, layer["InputWidth"], buffer)
        offset = serialization_save_uint32(offset, layer["InChannels"], buffer)
        offset = serialization_save_uint32(offset, layer["OutChannels"], buffer)
        offset = serialization_save_uint32(offset, layer["KernelSize"], buffer)
        offset = serialization_save_uint32(offset, layer["Stride"], buffer)
        offset = serialization_save_uint32(offset, layer["Padding"], buffer)
        offset = serialization_save_uint32(offset, layer["PaddingMode"], buffer)
        offset = serialization_save_float_array(offset, layer["Weights"], buffer)
        offset = serialization_save_float_array(offset, layer["Biases"], buffer)
...
def serialization_load_layer(offset, buffer):
    ...
    if layer_type == "Invalid":
        ...
        offset, output_size = serialization_load_uint32(offset, buffer)
        offset, remember_layer = serialization_load_layer(offset, buffer)
        offset, update_layer = serialization_load_layer(offset, buffer)
        offset, activation_layer = serialization_load_layer(offset, buffer)
        result = {
            "Type": layer_type,
            "InputSize": input_size,
            "OutputSize": output_size,
        ...
        offset, input_length = serialization_load_uint32(offset, buffer)
        offset, in_channels = serialization_load_uint32(offset, buffer)
        offset, out_channels = serialization_load_uint32(offset, buffer)
        offset, kernel_size = serialization_load_uint32(offset, buffer)
        offset, padding = serialization_load_uint32(offset, buffer)
        offset, padding_mode = serialization_load_uint32(offset, buffer)
        offset, weights = serialization_load_float_array(offset, buffer, [out_channels, in_channels, kernel_size])
        offset, biases = serialization_load_float_array(offset, buffer, [out_channels])
        offset, sublayer = serialization_load_layer(offset, buffer)
        result = {
            "Type": layer_type, 
        ...
        offset, input_height = serialization_load_uint32(offset, buffer)
        offset, input_width = serialization_load_uint32(offset, buffer)
        offset, in_channels = serialization_load_uint32(offset, buffer)
        offset, out_channels = serialization_load_uint32(offset, buffer)
        offset, kernel_size = serialization_load_uint32(offset, buffer)
        offset, stride = serialization_load_uint32(offset, buffer)
        offset, padding = serialization_load_uint32(offset, buffer)
        offset, padding_mode = serialization_load_uint32(offset, buffer)
        offset, weights = serialization_load_float_array(offset, buffer, [out_channels, in_channels, kernel_size, kernel_size])
        offset, biases = serialization_load_float_array(offset, buffer, [out_channels])
        offset, sublayer = serialization_load_layer(offset, buffer)
        result = {
            "Type": layer_type, 
...
def evaluate(layer, x):
    ...
    if layer_type == "Invalid":
        ...
        r = _sigmoid(evaluate(layer['RememberLayer'], x))
        z = _sigmoid(evaluate(layer['UpdateLayer'], x))
        x_mask = np.concatenate([np.ones([len(x), layer['InputSize']]), r], axis=-1)
        y = (1 - z) * x[:,layer['InputSize']:] + z * np.tanh(evaluate(layer['ActivationLayer'], x_mask * x))
        return np.concatenate([y, y], axis=-1)

    elif layer_type == "Conv1d":
        # x here should be (batch_size, N=in_channels*input_length)
        # weights (out_channels, in_channels, kernel_size)
        # biases (out_channels)
        sublayer_out = evaluate(layer['SubLayer'], x)

        batch_size = sublayer_out.shape[0]

        input_length = layer['InputLength']
        in_channels = layer['InChannels']
        out_channels = layer['OutChannels']
        kernel_size = layer['KernelSize']
        padding = layer['Padding']

        # Convolution Padding Modes - this should match what is in  NNERuntimeBasicCpuModel.cpp

        padding_modes = {
            0: "constant", # Zeros
            1: "wrap", # Circular
        } 
        padding_mode = padding_modes[layer['PaddingMode']]

        x = sublayer_out.reshape(batch_size, in_channels, input_length)
        x = np.pad(x, ((0,0), (0,0), (padding, padding)), mode=padding_mode)

        output_length = input_length + 2 * padding - kernel_size + 1
        windows = np.stack([x[:,:,i:i+output_length] for i in range(kernel_size)], axis=-1) # (batch_size, in_channels, output_length, kernel_size)

        convolution = np.tensordot(windows, layer['Weights'], axes=([1,3],[1,2]))
        y = np.transpose(convolution, (0,2,1)) + layer['Biases'][None, :, None] # (batch_size, out_channels, output_length)
        return y.reshape(batch_size, -1) # (batch_size, out_channels * output_length)

    elif layer_type == "Conv2d":
        # x here should be NCHW (batch_size, N=in_channels*input_height*input_width)
        ...
        in_height = layer['InputHeight']
        in_width = layer['InputWidth']
        in_channels = layer['InChannels']
        out_channels = layer['OutChannels']
        kernel_size = layer['KernelSize']
        stride = layer['Stride']
        ...
        # Convolution Padding Modes - this should match what is in  NNERuntimeBasicCpuModel.cpp
        # Only zeros is supported at the moment 
        padding_modes = {
            0: "constant", # Zeros
        } 
        padding_mode = padding_modes[layer['PaddingMode']]

        x = sublayer_out.reshape(batch_size, in_channels, in_height, in_width)
        x = np.pad(x, ((0,0), (0,0), (padding, padding), (padding, padding)), mode=padding_mode) # (N, in_channels, in_height + 2*padding, in_width + 2*padding)
        
        patches = sliding_window_view(x, window_shape=(kernel_size, kernel_size), axis=(2,3)) 
        
        windows = patches[:,:,::stride,::stride,:,:] # (batch_size, in_channels, out_height, out_width, kernel_size, kernel_size)

        convolution = np.tensordot(windows, layer['Weights'], axes=([1,4,5],[1,2,3]))
        y = np.transpose(convolution, (0,3,1,2)) + layer['Biases'][None, :, None, None] # transpose from NHWC to NCHW
        return y.reshape(batch_size, -1) # (batch_size, out_channels * out_height * out_width)
