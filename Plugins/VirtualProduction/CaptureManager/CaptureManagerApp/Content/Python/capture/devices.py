...
class StereoVideoIngestDeviceBase(LiveLinkHubCaptureDevice):
    """ An stereo video ingest device """
    ...
    def __init__(self, archive_path: str, video_discovery_expression: str, audio_discovery_expression: str, *, name: str = None, start_timeout: int = _DEFAULT_ARCHIVE_TIMEOUT):
        """
        :param archive_path: Path to the stereo take
        :param video_discovery_expression: Video file discovery expression
        :param audio_discovery_expression: Audio file discovery expression
        :param name: Device name
        :param start_timeout: Start timeout (in seconds)
        """
        device_name = name if name is not None else self._default_name()

        settings = unreal.StereoVideoIngestDeviceSettings()
        settings.display_name = device_name
        settings.video_discovery_expression.value = video_discovery_expression
        settings.audio_discovery_expression.value = audio_discovery_expression
        settings.take_directory.path = archive_path

        factory = unreal.LiveLinkHubCaptureDeviceFactory()
        device = factory.create_device_by_class(device_name, unreal.StereoVideoIngestDevice.static_class(), settings)
...
class StereoVideoIngestDevice(StereoVideoIngestDeviceBase):
    """ A convenience wrapper for Stereo Video Capture devices """
    ...
    def __init__(self, archive_path: str, video_discovery_expression: str, audio_discovery_expression: str, *, name: str = None, start_timeout: int = _DEFAULT_ARCHIVE_TIMEOUT):
        ...
        :param start_timeout: Start timeout (in seconds)
        """
        super().__init__(archive_path, video_discovery_expression, audio_discovery_expression, name=name, start_timeout=start_timeout)