...
from capture.devices import StereoVideoIngestDevice
...
def main():
    parser = argparse.ArgumentParser(description='Example Stereo Video Ingest device')
    parser.add_argument('--archive-path', required=True, type=str, help='Path to the stereo take')
    parser.add_argument('--video-discovery-expression', type=str, default='<Auto>', help='Format of the video files')
    parser.add_argument('--audio-discovery-expression', type=str, default='<Auto>', help='Format of the audio files')
    ...
    with UnrealEndpointManager(connect_timeout=10) as endpoint_manager:
        ...
        with StereoVideoIngestDevice(args.archive_path, args.video_discovery_expression, args.audio_discovery_expression) as stereo_video:
            ingest_takes(stereo_video, settings)
