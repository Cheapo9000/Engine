...
from switchboard.config import (
    ...
    EngineSyncMethod, FilePathSetting, FloatSetting, IntSetting, LoggingSetting, MultiOptionSetting, OptionSetting, Setting,
...
class UAssetSetting(Setting):
    '''
    A setting which points to an Unreal asset.

    Its main widget is a combo box that enumerates certain project/plugin asset types.
    This is an abstract base class; concrete subclasses override `_classnames()` to specify their uasset class names.

    Its "refresh" button triggers an asset traversal to refresh the options.
    '''
    ...
    @abstractmethod
    def _classnames(self) -> Iterable[str]:
...
class LiveLinkPresetSetting(UAssetSetting):
    ''' Container of the LiveLinkPreset setting.
    '''
    def _classnames(self) -> Iterable[str]:
        ''' Return the valid class names of this type of asset '''
...
class MediaProfileSetting(UAssetSetting):
...
class DeviceUnreal(Device):
    ...
    csettings: dict[str, Setting] = {
        ...
        'use_fast_sync': BoolSetting(
            attr_name='use_fast_sync',
            nice_name='Use Fast Sync',
            value=False,
            tool_tip='Controls whether Perforce sync operations use the revision range optimization',
        ...
        'soft_kill_timeout': FloatSetting(
            ...
            tool_tip=(
                ...
                'Applies to Unreal and nDisplay devices. '
                'Processes have this amount of time to save state and clean up before being forcefully killed.'
            ),
            category="General Settings",
        ),
    ...
    class StaticSignals(QtCore.QObject):
        ...
        clipboard_level_added_signal = QtCore.Signal(str)
    ...
    def p4_refresh_ugs_config(cls):
        if DeviceUnreal.ugs_config is None:
            # One-time initialization
            def refresh():
                DeviceUnreal.p4_refresh_ugs_config()
        ...
        if not CONFIG.P4_ENABLED.get_value():
            DeviceUnreal.ugs_config = ugs_utils.IniParser()
            return

        if DeviceUnreal._pending_ugs_config_refresh:
            return
        elif DeviceUnreal._ugs_config_runnable:
            DeviceUnreal._pending_ugs_config_refresh = True
            return

        engine_dir = CONFIG.P4_ENGINE_PATH.get_value() or None
        project_dir = CONFIG.P4_PROJECT_PATH.get_value() or None
        DeviceUnreal._ugs_config_runnable = DeviceUnreal.UgsConfigRunnable(
            engine_dir, project_dir)

        DeviceUnreal._ugs_config_runnable.signals.result.connect(
            DeviceUnreal._on_updated_ugs_config)
        QtCore.QThreadPool.globalInstance().start(DeviceUnreal._ugs_config_runnable)
    ...
    def __init__(self, name, address, **kwargs):
        ...
        # The CLs returned via syncstatus query, or optimistically
        # updated with the "inflight" values upon successful sync
        # (Defined in the base class, documented here for context)
        #self._project_changelist: Optional[int | str] = None
        #self._engine_changelist: Optional[int | str] = None
        ...
        if self.ugs_config is None and self._ugs_config_runnable is None:
    ...
    def _request_syncstatus(self):
        ...
        if CONFIG.ENGINE_SYNC_METHOD.get_value() == EngineSyncMethod.Build_Engine.value:
            syncstatus_args.append(f'--engine-dir="{engine_dir}"')

        if workspace:
            syncstatus_args.append(f'--p4client={workspace}')
        ...
        puuid, msg = message_protocol.create_start_process_message(
            prog_path=self.python_path,
            prog_args=' '.join(syncstatus_args),
            prog_name=program_name,
            caller=self.name,
            working_dir=working_dir,
            update_clients_with_stdout=False,
            combined_stdout_stderr=False,
            hide=True,
        )
    ...
    def sync(
        self,
        engine_cl: Optional[int | str],
        project_cl: Optional[int | str],
        ...
        if isinstance(engine_cl, str):
            try:
                engine_cl = int(engine_cl)
            except ValueError:
                LOGGER.error(f'Invalid engine changelist specified for sync: {engine_cl}')
                return None

        if isinstance(project_cl, str):
            try:
                project_cl = int(project_cl)
            except ValueError:
                LOGGER.error(f'Invalid project changelist specified for sync: {project_cl}')
                return None
        ...
        if not DeviceUnreal.csettings['use_fast_sync'].get_value():
            sync_args.append('--no-fast-sync')
    ...
    def _get_editor_target_name(self) -> Optional[str]:
        ''' Attempts to find the specific editor target name for the project, mimicking UGS.
        
        Uses hierarchical detection of the editor target:

            Priority 1: Config/DefaultEngine.ini
            Priotity 2: Source/{PROJECT}Editor.Target.cs
            Priority 3: Source/*Editor.Target.cs

        If not found, we expect the caller to use -Target=Editor in the UGS command line.

        Returns the full editor target name, or None if not found.
        '''
        ...
        try:
            uproject_file = pathlib.Path(uproject_path)
            ...
            # Priority 1: Config/DefaultEngine.ini
            # Looks for DefaultEditorTarget in its [/Script/BuildSettings.BuildSettings] section

            config_dir = uproject_file.parent / 'Config'
            engine_ini_path = config_dir / 'DefaultEngine.ini'
            ...
            if engine_ini_path.exists():
                try:
                    ...
                    default_target = parser.try_get_raw('/Script/BuildSettings.BuildSettings', 'DefaultEditorTarget')

                    if default_target:
                        LOGGER.debug(f"{self.name}: Found DefaultEditorTarget '{default_target}' in DefaultEngine.ini")
                        return default_target
                        
                except Exception as e:
                    LOGGER.warning(f"{self.name}: Failed to parse DefaultEngine.ini: {e}")
            
            # Priority 2: Source/{PROJECT}Editor.Target.cs

            source_dir = uproject_file.parent / 'Source'

            # No project-specific editor targets in blueprint only projects.
            if not source_dir.exists():
                return None
                
            file_suffix = '.Target.cs'
            expected_target_file = source_dir / f"{project_name}Editor{file_suffix}"
            
            if expected_target_file.exists():
                target_name = f"{project_name}Editor"
                LOGGER.debug(f"{self.name}: Found project-specific editor target: {target_name}")
                return target_name
            
            # Priority 3: Source/*Editor.Target.cs

            editor_targets = list(source_dir.glob(f'*Editor{file_suffix}'))
            
            if len(editor_targets) == 1:
                target_file = editor_targets[0]
                target_name = target_file.name[:-len(file_suffix)]
                LOGGER.debug(f"{self.name}: Found single editor target: {target_name}")
                return target_name
                    
            elif len(editor_targets) > 1:

                # Multiple targets found, but no clear winner.

                target_names = [f.name[:-len(file_suffix)] for f in editor_targets]

                LOGGER.warning(
                    f"{self.name}: Multiple editor targets found but no DefaultEditorTarget configured: {target_names}"
                )
                
        except Exception as e:
            LOGGER.warning(f"{self.name}: Failed to detect editor target name: {e}")
            
        return None

    def _build_project(self, puuid_dependency: Optional[uuid.UUID] = None) -> uuid.UUID | None:

        # Try to get specific project editor target name like UGS does
        # If found (e.g. MyProjectNameEditor), use it instead of `-TargetType=Editor`
        # Otherwise fall back to `-TargetType=Editor`

        editor_target = self._get_editor_target_name()
        
        if editor_target:
            ubt_args = (
                f'{editor_target} {self.target_platform} Development '
    ...
    def close(self, force=False, soft_kill_timeout=None):
        ...
        if soft_kill_timeout is None:
            soft_kill_timeout = DeviceUnreal.csettings['soft_kill_timeout'].get_value(self.name)

        for unreal_puuid in unreal_puuids:
            _, msg = message_protocol.create_kill_process_message(unreal_puuid, soft_kill_timeout)
            self.unreal_client.send_message(msg)
    ...
    def generate_unreal_command_line_args(self, map_name):
        ...
        command_line_args += (f'-CONCERTSERVER="{mu_server.configured_server_name()}" '
                              f'-CONCERTSESSION="{CONFIG.MUSERVER_SESSION_NAME.get_value()}" '
        ...
        # Disable ensures
        if DeviceUnreal.csettings['disable_ensures'].get_value(self.name):
            command_line_args += ' -handleensurepercent=0'
        ...
        command_line_args += f' -VPOSCServerAddress={self.address}'
        command_line_args += f' -VPOSCServerPort={self.device_osc_port}'

        # Logging
        logging_args = DeviceUnreal.csettings['logging'].get_command_line_arg(
            override_device_name=self.name)
        if logging_args:
            command_line_args += f' {logging_args}'
    ...
    def on_program_started(self, message):
        ...
        if not message['bAck']:
            ...
            if program_name == UnrealJobs.Sync.value or program_name.startswith(UnrealJobs.BuildPrefix.value):
                # Sync/Build failures may affect multiple devices
                for device in self.devices_sharing_workspace():
                ...
                # Any other program failure sets this device to CLOSED
                self.status = DeviceStatus.CLOSED
    ...
    def on_program_ended(self, message):
        ...
        def get_stderr_str() -> str:
            b64bytes = base64.b64decode(message['stderrB64'])
            return b64bytes.decode()
    ...
    def do_program_ended_update(
        ...
        *,
        ...
        elif program_name == UnrealJobs.SyncStatus.value:
            output = get_stdout_str()

            try:
                syncstatus: dict = json.loads(output)
            except Exception:
                stderr = ''
                try:
                    stderr = get_stderr_str()
                except KeyError:
                    pass

                LOGGER.error(f'{self.name}: Error parsing syncstatus output: {output}')

                if stderr:
                    LOGGER.error(f'{self.name}: stderr: {stderr}')

                LOGGER.error(f'{self.name}: Could not retrieve sync status. '
                             'Are the source control settings correctly configured?')

                for device in self.devices_sharing_workspace():
                    if CONFIG.ENGINE_SYNC_METHOD.get_value() == EngineSyncMethod.Build_Engine.value:
                        device.engine_changelist = '(unknown)'
                    device.project_changelist = '(unknown)'

                return

            for device in self.devices_sharing_workspace():
                dirty_str = '*' if syncstatus.get('dirty') else ''
                if engine_cl := syncstatus.get('engine_cl'):
                    device.engine_changelist = f'{engine_cl}{dirty_str}'
                if project_cl := syncstatus.get('project_cl'):
                    device.project_changelist = f'{project_cl}{dirty_str}'
    ...
    def on_listener_programstdout(self, message):
        ...
        if process['name'].startswith(UnrealJobs.BuildPrefix.value):
            ...
            for line in lines:
                ...
                log_fn(f"{self.name} {process['name']}: {line}")
    ...
    def analyze_project_assets(cls):
        ...
        find_canceled = progressDiag.wasCanceled()
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            progressDiag.canceled.connect(lambda: executor.shutdown(cancel_futures=True))
    ...
    @classmethod
    def handle_clipboard_mime_text(cls, text:str):
        ''' Catches pasting of content browser assets.
        The main purpose is to auto-add them to switchboard's caches of assets and levels.

        Expected syntax:
            /Script/Module.Class'Gamepath.Name'
        
        E.g. umap:
            /Script/Engine.World'/Game/Tests/AA/LLF/L_LLF.L_LLF

        When multiple objects are selected, they are listed one per line.
        '''  
        asset_reference_re = re.compile(r"""
            ^/Script/                              # /Script/
            (?P<module>[A-Za-z_]\w+)               # Module
            \.                                     # .
            (?P<classname>[A-Za-z_]\w+)            # Class
            '                                      # '
            (?P<gamepath>[A-Za-z0-9_/]+)           # Gamepath
            \.                                     # .
            (?P<name>[A-Za-z0-9_]+)                # Name
            '                                      # '
            $""", re.VERBOSE)
        ...
        # When selecting multiple assets, they are listed on separate lines.
        for line in text.splitlines():

            match = asset_reference_re.match(line)

            if not match:
                LOGGER.warning(f"Invalid asset reference: {line!r}")
                continue
            
            module    = match.group("module")
            classname = match.group("classname")
            gamepath  = match.group("gamepath")
            name      = match.group("name")

            namespaced_classname = f"/Script/{module}.{classname}"

            if namespaced_classname == "/Script/Engine.World":
                levels.append(gamepath)

            elif namespaced_classname in INTERESTING_CLASS_NAMES:
                filepath = CONFIG.ue_plugin_mgr.content_to_file_path(gamepath)

                # The class type implies the file extension.
                uasset_ext = '.uasset'

                file_exists = False

                if filepath:
                    # Append the extension it since content_to_file_path(..) doesn't.
                    filepath = str(filepath) + uasset_ext
                    file_exists = os.path.exists(filepath)
                else:
                    filepath = ''

                if not file_exists:
                    # Don't save invalid filepaths
                    filepath = ''

                    # The only case that requires a valid filepath is nDisplay configs, since they are parsed
                    # during launch. The other asset references are ok with just a valid gamepath.
                    if namespaced_classname in DeviceUnreal.NDISPLAY_CLASS_NAMES:
                        LOGGER.warning(f"Skipping nDisplay config '{gamepath}' because its file on disk could not be located")
                        continue
                    else:
                        LOGGER.warning(f"Could not resolve to an existing filepath for '{gamepath}'")

                assettype = SBCache().query_or_create_assettype(namespaced_classname)
                asset_name = name + uasset_ext
        ...
        if len(levels):
            ...
            reply = QtWidgets.QMessageBox.question(
                None,
                "Add Unreal Maps",
                f"The following maps will be added:\n\n{levels_text}",
                QtWidgets.QMessageBox.Ok | QtWidgets.QMessageBox.Cancel
            )

            if reply == QtWidgets.QMessageBox.Ok:
                for level in levels:
                    CONFIG.add_single_level(level)
                    cls.static_signals.clipboard_level_added_signal.emit(level)
        ...
        if len(assets):
            ...
            reply = QtWidgets.QMessageBox.question(
                None,
                "Add Unreal Assets",
                f"The following assets will be added:\n\n{assets_text}",
                QtWidgets.QMessageBox.Ok | QtWidgets.QMessageBox.Cancel
            )

            SBCache().merge_project_assets(project, assets)

            for asset in assets:
                LOGGER.info(f"Added '{asset.name}'")
...
class DeviceWidgetUnreal(DeviceWidget):
    ...
    def update_status(self, status, previous_status, device=None):
        super().update_status(status, previous_status, device)
    ...
    def update_project_changelist(self, required_cl: str | None, current_device_cl: str):
        ...
        try:
            required_cl_int = int(required_cl)
