...
class UnrealAutoConfigScript(SwitchboardScriptBase):
    ...
    def on_postinit(self, app: SwitchboardDialog):
        ...
        # Note: There is no need to CONFIG.PROJECT_NAME.update_value(f"{Path(self.params['projectPath']).stem}")
        # because app.create_new_config(..) will generate it based on name of the config, which is
        # consistent with creating new configs manually.

        # add and select map
        CONFIG.add_single_level(self.params['map'])
        app.refresh_levels(CONFIG.get_levels())
