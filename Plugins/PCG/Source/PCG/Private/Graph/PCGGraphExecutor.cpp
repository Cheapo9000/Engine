...
FPCGGraphExecutor::FPCGGraphExecutor(UWorld* InWorld)
	: GraphCache(InWorld && InWorld->IsGameWorld())
	, World(InWorld)
{
...
FPCGTaskId FPCGGraphExecutor::ScheduleGraph(const FPCGScheduleGraphParams& InParams)
{
	...
	if (IPCGBaseSubsystem* Subsystem = InParams.ExecutionSource->GetExecutionState().GetSubsystem())
	{
...
IPCGBaseSubsystem* FPCGGraphExecutor::GetSubsystem() const
{
	if (World)
	{
...
{
	// Only check state on game thread
	if (!bIsInGameThread)
	{
		return;
	}

	// ScheduledTasks needs to be empty, check without a lock to early out if that is not the case
	...
	const double ElapsedTimeSinceLastCheck = TimeInSeconds - LastSchedulingErrorCheck;
	if (ElapsedTimeSinceLastCheck < 1.0)
	{
		return;
	}

	// Since this is for an error case avoid locking if lock is already taken, we can comeback later but if we haven't checked in more than 3 seconds actually lock
	PCGGraphExecutor::TScopeLock ScheduleScopeLock(ScheduleLock, /*bTryLock=*/ ElapsedTimeSinceLastCheck < 3.0);
	if (!ScheduleScopeLock.IsLocked())
	{
		return;
	}
...
void FPCGGraphExecutor::PostTaskExecute(TSharedPtr<FPCGGraphActiveTask> ActiveTaskPtr, bool bIsDone)
{
	...
	if (!bIsDone || !ActiveTask.Context->DynamicDependencies.IsEmpty())
	{
		...
		if (!ActiveTaskPtr->bWasCancelled)
		{
			...
			// We have dynamic dependencies bIsPaused flag might not have been set by the element
			// It is safe to set bIsPaused here because tasks with DynamicDependencies can't be unpaused by another task (race-condition)
			if (bHasPausedDependencies)
			{
				ActiveTask.Context->bIsPaused = true;
			}

			// Still paused (because of multithreading it is possible that we were unpaused before processing the paused state)
	...
	const bool bTaskFullyExecuted = !ActiveTask.bWasCancelled && ActiveTask.Context->CurrentPhase == EPCGExecutionPhase::Done && ActiveTask.Context->DynamicDependencies.IsEmpty();
...
#endif
					{
						check(ActiveTask->Context->DynamicDependencies.IsEmpty());
...
void FPCGGraphExecutor::Execute(double& InOutEndTime)
{
	...
	if (bOnWorldGenerationProcessBroadcasted)
	{
		FEditorDelegates::OnPostWorldGenerationProcess.Broadcast(World);
	}
#endif // WITH_EDITOR
}
...
void FPCGGraphExecutor::BuildTaskInput(FPCGGraphTask& Task, TArray<FPCGTaskId>& ResultsToMarkAsRead)
{
	...
	if(Task.Inputs.Num() > 0)
	{
		...
		for (const FPCGGraphTaskInput& Input : Task.Inputs)
		{
			...
			if (!Input.bProvideData || (Input.DownstreamPin.IsSet() && Input.DownstreamPin.GetValue().IsDatalessPin()))
			{
				continue;
			}

			const bool bAllowMultipleData = Input.DownstreamPin.IsSet() ? Input.DownstreamPin.GetValue().bAllowMultipleData : true;
			const uint32 InputPinLabelCrc = Input.DownstreamPin.IsSet() ? FCrc::StrCrc32(*Input.DownstreamPin.GetValue().Label.ToString()) : DefaultHashForNoOutputPin;
...
{
	const IPCGGraphExecutionSource* ExecutionSource = InStack ? InStack->GetRootSource() : nullptr;
	if (ExecutionSource && InNode)
	{
		ExecutionSource->GetExecutionState().GetInspection().NotifyNodeDynamicInactivePins(InNode, InStack, InactiveOutputPinBitmask);
	}
}
#endif
...
void FPCGGraphExecutor::AddReferencedObjects(FReferenceCollector& Collector)
{
	...
	PerExecutionCache.AddStructReferencedObjects(Collector);
...
{
	// Avoid notifications for commandlets
	if (IsRunningCommandlet())
	{
		return;
	}

	// Avoid notifications for non world / non simulating game world executors unless they are standalone executors
	if (!bIsStandaloneGraphExecutor && (!World || (World->IsGameWorld() && GEditor && !GEditor->IsSimulateInEditorInProgress())))
	{
		return;
	}
...
void FPCGGraphExecutor::OnNotificationCancel()
{
	...
	if (IPCGBaseSubsystem* PCGSubsystem = GetSubsystem())
	{
...
bool FPCGPreGraphElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	check(IsInGameThread());
	const bool bIsValid = Context->ExecutionSource.IsValid();
	IPCGGraphExecutionSource* ExecutionSource = Context->ExecutionSource.Get();

	// Early out if the execution source has been deleted/is invalid
	if (!ExecutionSource)
	{
		// If the execution source should exist but it doesn't (which is all the time here, previously we checked for it), then this should be cancelled
		Context->OutputData.bCancelExecution = !bIsValid;
		return true;
	}

	ExecutionSource->GetExecutionState().ExecutePreGraph(Context);
...
bool FPCGFetchInputElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	// Skip retrieval if the graph is marked as not providing default inputs
	if (const UPCGGraph* Graph = Context->Node->GetGraph())
	{
		if (!Graph->HasDefaultConstructedInputs())
		{
			return true;
		}
	}
...
namespace PCGGraphExecutor
{
		...
		FPCGGridLinkageContext* InContext)
	{
		...
		if (!ExecutionState.GetGraph()->IsHierarchicalGenerationEnabled()
		...
		if (!!(InFromGrid & InGenerationGrid) && FromGridSize != ToGridSize)
		{
			...
			ExecutionState.StoreOutputDataForPin(InResourceKey, Data);
		}
		else if (InToGrid == InGenerationGrid && FromGridSize != ToGridSize)
		{
			...
			IPCGGraphExecutionSource* ExecSourceWithData = nullptr;
			bool bExecSourceWithDataIsOriginal = false;

			if (FromGridSize == PCGHiGenGrid::UnboundedGridSize())
			{
				ExecSourceWithData = ExecutionState.GetOriginalSource();
				bExecSourceWithDataIsOriginal = true;
			}
			else
			{
				const IPCGGraphExecutionSource* OriginalSource = ExecutionState.GetOriginalSource();
				if (!ensure(OriginalSource))
				{
					return false;
				}

				const FPCGGridDescriptor GridDescriptor = OriginalSource->GetExecutionState().GetGridDescriptor(FromGridSize);

				// Get grid coords using the parent grid (FromGridSize).
				const FIntVector CellCoords = UPCGActorHelpers::GetCellCoord(ExecutionState.GetTransform().GetLocation(), GridDescriptor.GetGridSize(), GridDescriptor.Is2DGrid());

				ExecSourceWithData = ExecutionState.GetLocalSource(GridDescriptor, CellCoords);
			}

			if (!ExecSourceWithData)
			{
				// Nothing we can do currently if execution source is not present. One idea is to schedule an artifact-less execution but that
				// comes with complications - artifacts/side effects are an integral part of execution. Most likely we'll do a cleanup
				// pass of any unwanted artifacts/local-execution sources later.
				PCGGraphExecutionLogging::LogGridLinkageTaskExecuteRetrieveNoLocalSource(InContext, InResourceKey);
				return true;
			}

			// Once we've found our execution source, try to retrieve the data.
			if (const FPCGDataCollection* Data = ExecSourceWithData->GetExecutionState().RetrieveOutputDataForPin(InResourceKey))
			{
				PCGGraphExecutionLogging::LogGridLinkageTaskExecuteRetrieveSuccess(InContext, ExecSourceWithData, InResourceKey, Data->TaggedData.Num());
			...
			// If we need data from a local execution source but the local execution source is still generating, then we'll wait for it. On the other hand
			// if we need data from the original execution source we assume the generation has already happened because it is always scheduled before
			// the local execution source.
			const bool bWaitForGeneration = bExecSourceWithDataIsOriginal ? false : ExecSourceWithData->GetExecutionState().IsGenerating();
			if (bWaitForGeneration)
			{
				PCGGraphExecutionLogging::LogGridLinkageTaskExecuteRetrieveWaitOnScheduledGraph(InContext, ExecSourceWithData, InResourceKey);

				// The execution source was already generating, but we were not paused. Not really clear what's happening here,
				...
				FPCGTaskId GenerationTask = ExecSourceWithData->GetExecutionState().GetGenerationTaskId();
				if (ensure(GenerationTask != InvalidPCGTaskId))
				{
					InContext->DynamicDependencies.Add(GenerationTask);
				}
			...
			// But don't do this for the original execution source as this will always be scheduled before the local execution source.
			if (!InContext->bScheduledGraph && !bExecSourceWithDataIsOriginal)
			{
				PCGGraphExecutionLogging::LogGridLinkageTaskExecuteRetrieveScheduleGraph(InContext, ExecSourceWithData, InResourceKey);

				// Wake up this task after graph has generated.
				const FPCGTaskId GraphTaskId = ExecSourceWithData->GetExecutionState().GenerateLocalGetTaskId();
			...
			else
			{
				...
				PCGGraphExecutionLogging::LogGridLinkageTaskExecuteRetrieveNoData(InContext, ExecSourceWithData, InResourceKey);
				return true;
			}
		}
		else
