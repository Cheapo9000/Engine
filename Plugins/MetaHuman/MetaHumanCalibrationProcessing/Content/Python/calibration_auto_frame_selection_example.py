...
def main():
    ...
    frame_selector = unreal.MetaHumanCalibrationAutoFrameSelector()
    
    calib_options.set_selected_frames(frame_selector.run(capture_data_asset, calib_config, calib_options))
    
    if not calib_options.selected_frames:
        raise RuntimeError(f'Failed to select the frames for capture data asset : {args.capture_data_package_path}')
