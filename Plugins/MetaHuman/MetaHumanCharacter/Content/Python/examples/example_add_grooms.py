...
groom_wardrobe_item = unreal.load_asset(
    "/MetaHumanCharacter/Optional/Grooms/Bindings/Hair/WI_Hair_L_StraightBangs.WI_Hair_L_StraightBangs"
)

item_key = character.internal_collection.try_add_item_from_wardrobe_item(
    "Hair", groom_wardrobe_item
)
...
try:
    ...
    groom_item_path = unreal.MetaHumanPaletteItemPath(item_key=item_key)

    groom_instance_parameters = character.internal_collection.default_instance.get_instance_parameters(item_path=groom_item_path)
    if not (melanin_param := next((param for param in groom_instance_parameters if param.name == "Melanin"), None)):
        unreal.log_error("Failed to find Melanin parameter")
        raise RuntimeError
